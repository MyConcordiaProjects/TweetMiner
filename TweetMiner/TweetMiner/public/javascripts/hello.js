;(function(window) {
	...
	jQuery('#search-form').submit(function (searchfunc) {
        ...
        if( searchKeyword.value != '' ){
        	var heading = "";
        	heading += "<h2>Recent tweets related to <i>" + searchKeyword.value + "</i></h2>"
        	/*
			 * jQuery.get("http://localhost:9000/tweetminer/" +
			 * searchKeyword.value).done(function (allTweets) {
			 * tweetlist.concat(allTweets); allTweets.forEach(function
			 * (eachTweet) { tweets += "<p>=> "; if(eachTweet.place &&
			 * eachTweet.latitude && eachTweet.longitude) { tweets += "<a
			 * target='_blank'
			 * href='http://localhost:9000/userProfile/"+eachTweet.displayName+"'>
			 * ("+eachTweet.userName+")</a>" + "<a target='_blank'
			 * href='http://localhost:9000/location/" +eachTweet.latitude +
			 * "/"+eachTweet.longitude+"'> ("+eachTweet.place+")</a>" +"Tweet -
			 * "+eachTweet.tweet; } else { tweets += "<a target='_blank'
			 * href='http://localhost:9000/userProfile/"+eachTweet.displayName
			 * +"'>("+eachTweet.userName + ")</a>" + ":" + eachTweet.tweet + "<a
			 * target='_blank' href='http://localhost:9000/hashtag/" +
			 * eachTweet.hashTag + "'>#" +eachTweet.hashTag + "</a>"; } tweets += "</p>";
			 * }); tweets += "<hr>";
			 */
        		...
        		var websocket = new WebSocket('ws://localhost:9000/reactivePage');
            	...
            	websocket.onmessage = function(message) {
                    ...
                    var tweets = "";
                    tweetsJSON.forEach(function (eachTweet) {
